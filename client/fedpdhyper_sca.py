...
class fedpdhyper_sca(Client):
    def __init__(self, device, model_func, received_vecs, dataset, lr, args):   
        super(fedpdhyper_sca, self).__init__(device, model_func, received_vecs, dataset, lr, args)
        ...
        if self.args.dataset == "AG_News":
            for k in range(self.args.local_epochs):
                for i, (inputs, labels, lengths) in enumerate(self.dataset):
                    ...
                    loss_correct = torch.sum((param_list + delta_list) * (param_list +delta_list))
                    delta_list1 = self.received_vecs['Local_VR_correction'].to(self.device)
                    loss_correct1 = torch.sum(param_list * delta_list1)
                    loss = loss_pred + self.args.lamb / 2 * loss_correct + loss_correct1
